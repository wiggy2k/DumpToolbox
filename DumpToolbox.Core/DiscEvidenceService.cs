...
ORDER BY s.source_path,i.display_name,e.path";
    ...
    private async Task<ImageEvidence> InspectImageAsync(string path, SkeletoolEvidenceImage info, IProgress<string>? log, CancellationToken ct)
    {
        ...
        UdfStructureEvidence udfStructures = udf
            ? UdfStructureInspector.Inspect(path, ct)
            : UdfStructureEvidence.Empty;
        ...
        // UDF is valid on CD-R/CD-RW as well as DVD/BD media. Presence of UDF
        // therefore must not override the geometry-based media classification.
        string media = ClassifyMediaForEvidence(logicalSectors, fs.Length);
        if (primary is null)
        {
            log?.Report($"    media={media}; ISO9660 PVD not found; UDF={(udf?"yes":"no")}; " +
                $"UDF descriptors={udfStructures.Descriptors.Count:N0}; VAT generations={udfStructures.Vats.Count:N0}");
            return new(media, udf, null, null, descriptors, [], [], [], [], [], [], udfStructures);
        }
        ...
        List<DiscMasteringObservation> mastering = await InspectMasteringObservationsAsync(
            reader, descriptors, iso, jol, ct).ConfigureAwait(false);
        log?.Report($"    media={media}; udf={(udf?"yes":"no")}; descriptors={descriptors.Count:N0}; ISO={iso.Count:N0}; Joliet={jol.Count:N0}; path-table records={pathTables.Count:N0}; pairs={pairs.Count:N0}; EOF={eofs.Count:N0}; mastering observations={mastering.Count:N0}; UDF descriptors={udfStructures.Descriptors.Count:N0}; VAT generations={udfStructures.Vats.Count:N0}");
        return new(media,udf,primary,joliet,descriptors,iso,jol,pathTables,pairs,eofs,mastering,udfStructures);
    }
    ...
    internal static string ClassifyMediaForEvidence(long logicalSectors, long imageBytes)
        => logicalSectors > 450_000 || imageBytes > 900L * 1024 * 1024 ? "DVD" : "CD";
...
CREATE INDEX IF NOT EXISTS ix_name_pairs_image ON name_pairs(image_id); CREATE INDEX IF NOT EXISTS ix_eof_image ON eof_observations(image_id); INSERT OR IGNORE INTO meta(key,value) VALUES('schema_version','1');";
    ...
    private async Task StoreImageEvidenceAsync(SkeletoolEvidenceUnit unit,SkeletoolEvidenceImage image,ImageEvidence ev,CancellationToken ct)
    {
        ...
        await StoreUdfEvidenceAsync(db, tx, imageId, ev.UdfStructures, ct).ConfigureAwait(false);
    ...
    private sealed record ImageEvidence(string Media,bool UdfPresent,DiscVolumeDescriptorEvidence? Primary,
        ...
        List<DiscMasteringObservation> MasteringObservations,UdfStructureEvidence UdfStructures);
    ...
    private sealed class SectorReader
    {
        ...
        public long LogicalSectorCount => _fs.Length / PhysicalSectorSize;
